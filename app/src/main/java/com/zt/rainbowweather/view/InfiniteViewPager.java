...
public class InfiniteViewPager extends RelativeLayout {
    ...
    public void setNewData(String s){
        if(!TextUtils.isEmpty(s)){
            setCurrentItemOfData(s);
//            setCurrentItemOfData(s, false);
//            ViewHolder holder = viewHolderCreator.getFreeViewHolder();
//            dataMap.put(position, object);
//            Object object = getDataOfItem(position);
//            holder.setData(object);
//            viewHolderCreator.usedViewHolders.put(MID_PAGES_INDEX + dataMap.size(), holder);
    ...
    public void setCurrentItemOfData(Object data) {
        ...
        for (Integer key : dataMap.keySet()) {
            Log.e(TAG, "setCurrentItemOfData: "+dataMap.get(key));
            if (data.equals(dataMap.get(key))) {
                scrollToItem(key, false);
                i++;
                break;
            }
        }
        ...
        if(i == 0){
            dataMap.put(MID_PAGES_INDEX + dataMap.size(), data);
//            scrollToItem(MID_PAGES_INDEX + dataMap.size(), false);
            setCurrentItemOfData(data, false);
        }
    }
