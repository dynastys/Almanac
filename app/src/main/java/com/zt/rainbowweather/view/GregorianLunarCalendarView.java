...
public class GregorianLunarCalendarView extends LinearLayout implements NumberPickerView.OnValueChangeListener{
    ...
    private void initValuesForM(ChineseCalendar cc, boolean isGregorian, boolean anim){
        ...
        if(isGregorian){
            ...
            @SuppressLint("WrongConstant") int monthLeap = Util.getMonthLeapByYear(cc.get(ChineseCalendar.CHINESE_YEAR));
            if(monthLeap == 0){
    ...
    private void initValuesForD(ChineseCalendar cc, boolean isGregorian, boolean anim){
        if(isGregorian){
            ...
            @SuppressLint("WrongConstant") int dayStop = Util.getSumOfDayInMonthForLunarByMonthLunar(cc.get(ChineseCalendar.CHINESE_YEAR), cc.get(ChineseCalendar.CHINESE_MONTH));
            @SuppressLint("WrongConstant") int daySway = cc.get(ChineseCalendar.CHINESE_DATE);
