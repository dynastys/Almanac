...
public class AutoVerticalScrollTextViewUtil {
    ...
    private List<Nnotice.DataBean> mDatas = new ArrayList<>();// 需要轮播的数据源
    private Nnotice.DataBean dataBean;//当前的数据

    private CharSequence title;// 需要改变TextView上面的文本内容

    public void setmDatas(List<Nnotice.DataBean> mDatas) {
        this.mDatas.clear();
        this.mDatas = mDatas;
    }

    // 构造方法
    public AutoVerticalScrollTextViewUtil(final AutoVerticalScrollTextView textView, List<Nnotice.DataBean> datas) {
        this.mDatas = datas;
                ...
                try {
                    if (mDatas == null || mDatas.size() <= 0) {
                        ...
                        while (isRunning) {
                            ...
                            title = mDatas.get(currentPos).getTitle();
    ...
    public interface OnMyClickListener {
        void onMyClickListener(int position, Nnotice.DataBean dataBean);
    }
