...
public class AutoVerticalScrollTextViewUtil {
    ...
    private List<WithdrawalRecord.DataBean.WithdrawalsInfoVmsBean> mDatas = new ArrayList<>();// 需要轮播的数据源
    private WithdrawalRecord.DataBean.WithdrawalsInfoVmsBean dataBean;//当前的数据

    private CharSequence title;// 需要改变TextView上面的文本内容

    public void setmDatas(List<WithdrawalRecord.DataBean.WithdrawalsInfoVmsBean> mDatas) {
        this.mDatas.clear();
        this.mDatas = mDatas;
    }

    // 构造方法
    public AutoVerticalScrollTextViewUtil(final AutoVerticalScrollTextView textView, List<WithdrawalRecord.DataBean.WithdrawalsInfoVmsBean> datas) {
        this.mDatas = datas;
        ...
        this.textView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (onMyClickListener != null)
                    onMyClickListener.onMyClickListener(currentPos, dataBean);
            }
        });
    }
                ...
                try {
                    if (mDatas == null || mDatas.size() <= 0) {
                        ...
                        while (isRunning) {
                            ...
                            title = mDatas.get(currentPos).getName()+"提取"+mDatas.get(currentPos).getGold()+"金币";
                            dataBean = mDatas.get(currentPos);
                            number++;
                            // 改变文本显示
    ...
    public interface OnMyClickListener {
        void onMyClickListener(int position, WithdrawalRecord.DataBean.WithdrawalsInfoVmsBean dataBean);
    }
